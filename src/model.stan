...
transformed parameters {
  ...
  phi_d[1]      <- 0;
  phi_d[2:D]    <- mu_phi_d + phi_d_raw * sigma_phi_d;
  lambda_d[1]   <- 0;
  lambda_d[2:D] <- mu_lambda_d + lambda_d_raw * sigma_lambda_d;
...
model {  
	...
	mu_phi_d       ~ normal(0, 2);
	sigma_phi_d    ~ normal(0, 2);
	mu_lambda_d    ~ normal(0, 2);
	sigma_lambda_d ~ normal(0, 2);
	...
	phi_d_raw    ~ normal(0, 1);     // implies phi_d ~ N(mu_phi_d, sigma_phi_d)
	lambda_d_raw ~ normal(0, 1);  // implies lambda_d ~ N(mu_lambda_d, sigma_lambda_d)
